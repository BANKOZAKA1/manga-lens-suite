...
class TranslationPreferences(
     ...
     */
    val provider = preferenceStore.getString("translation_provider", "custom")

    /** 選用的 model id（空＝用該 provider 的預設模型）。可手填或從「抓取模型」清單選（引擎 `LlmModels.list`）。 */
    val model = preferenceStore.getString("translation_model", "translategemma:4b")

    /** 自架 / 自訂 provider（sakura/custom）的 API base，例 `http://192.168.1.5:8080/v1`；其餘 provider 用內建端點時忽略。 */
    val apiBase = preferenceStore.getString("translation_api_base", "http://192.168.1.8:8765/v1")
     ...
     */
    val ignoreSfx = preferenceStore.getBoolean("translation_ignore_sfx", true)
    ...
    companion object {
        ...
        const val DEFAULT_TARGET_LANG = "Thai (natural dialogue used in Thai manga translations)"
