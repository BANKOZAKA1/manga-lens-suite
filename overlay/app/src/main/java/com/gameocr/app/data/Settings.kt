...
@Serializable
data class Settings(
    val baseUrl: String = "http://192.168.1.2:8765/v1/",
    val apiKey: String = "",
    val model: String = "translategemma:4b",
    ...
    val overlayStyleMode: OverlayStyleMode = OverlayStyleMode.ADAPTIVE,
    ...
    */
    ...
    val translatorEngine: TranslatorEngine = TranslatorEngine.GOOGLE_ML_KIT,
    ...
    companion object {
         ...
         */
        const val DEFAULT_PROMPT: String = """คุณคือนักแปลมังงะและมันฮวาระดับมืออาชีพ แปลข้อความ{source}ต่อไปนี้เป็น{target} โดย:
1. รักษาชื่อตัวละคร สถานที่ คำเรียก และคำเฉพาะให้สม่ำเสมอ;
2. ใช้ภาษาไทยเป็นธรรมชาติ เข้ากับอารมณ์และระดับภาษาของบทสนทนา;
3. รักษาลำดับบรรทัดและบริบทของทั้งหน้า;
4. ส่งออกเฉพาะคำแปล ไม่ใส่คำอธิบายหรือเครื่องหมายคำพูดเพิ่ม
ต้นฉบับ:
"""
...
object TranslationPresetCatalog {
    ...
    const val BUILTIN_MANGA_JA_ZH_NAME: String = "Manga ญี่ปุ่น → ไทย (ออฟไลน์)"
    const val BUILTIN_MANHWA_KO_TH: String = "builtin_manhwa_ko_th"
    const val BUILTIN_MANHWA_KO_TH_NAME: String = "Manhwa เกาหลี → ไทย (ออฟไลน์)"
    ...
    fun builtIns(): List<TranslationPreset> = listOf(
        ...
        fromSettings(
            id = BUILTIN_MANHWA_KO_TH,
            name = BUILTIN_MANHWA_KO_TH_NAME,
            shortName = "KO→TH",
            settings = Settings().copy(
                sourceLang = "ko",
                targetLang = "th",
                ocrEngine = OcrEngineKind.ML_KIT_KOREAN,
                translatorEngine = TranslatorEngine.GOOGLE_ML_KIT,
                overlayStyleMode = OverlayStyleMode.ADAPTIVE,
    ...
    private val builtInIds: Set<String> = setOf(BUILTIN_MANGA_JA_ZH, BUILTIN_MANHWA_KO_TH)
}
